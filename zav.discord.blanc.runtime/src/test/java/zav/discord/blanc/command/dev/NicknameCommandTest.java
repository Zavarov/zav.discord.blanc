...
public class NicknameCommandTest extends AbstractDevCommandTest {
  @BeforeEach
  public void setUp() {
    command = parse("b:dev.nickname");
  }
  
  @Test
  public void testCommandIsOfCorrectType() {
    assertThat(command).isInstanceOf(NicknameCommand.class);
  }
  
  @Test
  public void testSetNickname() throws Exception {
    command = parse("b:dev.nickname foo %s", guildId);

    command.run();
  
    ArgumentCaptor<String> nameCaptor = ArgumentCaptor.forClass(String.class);
  
    verify(selfMemberView, times(1)).setNickname(nameCaptor.capture());
