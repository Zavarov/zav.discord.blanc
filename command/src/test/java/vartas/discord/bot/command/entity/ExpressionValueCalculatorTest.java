...
public class ExpressionValueCalculatorTest extends AbstractTest {
    ...
    protected ASTExpressionType parse(String expression){
        try{
            ...
            Optional<ASTExpressionType> optional = parser.parse_StringExpressionType(expression);
            if(parser.hasErrors())
    ...
    protected BigDecimal valueOf(String expression){
        return ExpressionValueCalculator.valueOf(parse(expression).getExpression());
    }
