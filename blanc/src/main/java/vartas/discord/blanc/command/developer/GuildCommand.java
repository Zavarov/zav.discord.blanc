...
public class GuildCommand extends GuildCommandTOP{
    ...
    /**
     * Goes through the bots on all shards and gathers the guilds.
     */
    @Override
    public void run(){
        InteractiveMessage.Builder builder = new InteractiveMessage.Builder(channel, author, communicator);
        List<String> guilds = environment
                .guilds()
                .stream()
                .map(guild -> String.format("%s by %s", guild.getName(), guild.getOwner().getAsMention()))
